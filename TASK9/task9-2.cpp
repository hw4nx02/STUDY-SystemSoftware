...
void TParse(string record, Memory &memory) {
    ...
    if (len == objcode.length()) { // 재배치 비트 방식 사용했다면
...
void MParse(string record, Memory &memory) {
    ...
    for (int i = 0; i < count; i++) { // 홀수로 들어와도 처리하기 위해 nibbles + 1
        stringstream ss;
        ss << setw(2) << setfill('0') << hexToHexstr((unsigned char)memory.mem[address + i]);
        string tmp = ss.str();
        targetStr += tmp;
    }
    ...
    for (int i = 0; i < (count); i++) {
        memory.mem[address + i] = (target >> 8*(count-i-1)) & 0xFF;
    }

    // 수정 시작 주소의 값은 하프 니블 수정
    string secondHalfNibble = hexToHexstr((unsigned char)memory.mem[address]).substr(0);
    string resultNibble = firstHalfNibble + secondHalfNibble;
    memory.mem[address] = hexstrToHex(resultNibble);
}
